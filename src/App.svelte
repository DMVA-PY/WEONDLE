...
<script lang="ts">
  ...
  mode.subscribe((m) => {
    ...
    const randomWordIndex = seededRandomInt(
      0,
      words.words.length,
      modeData.modes[m].seed
    );
    word = words.words[randomWordIndex].word; // Access the word property from the object
    ...
    // Set the letter states so the keyboard displays correctly for the loaded game state
